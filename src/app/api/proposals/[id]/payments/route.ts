...
export async function POST(request: NextRequest, { params }: Ctx) {
  ...
  const installmentsPerCycle = Number(body?.installmentsPerCycle) || 1;
  ...
  const plan = generatePaymentPlan({
    recurrence,
    occurrences,
    installmentsPerCycle,
    cycleAmount: Math.round(cycleAmount),
    firstDueDate: typeof body?.firstDueDate === 'string' && body.firstDueDate ? body.firstDueDate : null,
  });
