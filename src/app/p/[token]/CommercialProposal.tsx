...
export function CommercialProposal({ initial, template, token, status, note, revision }: { initial: QuoteView; template: string; token: string; status: string; note: string | null; revision: string }) {
  ...
  return <><div className="p-4 sm:p-8"><CommercialChoices q={q} disabled={locked} onChange={(commercial, ids) => setQ({ ...q, commercial, items: q.items.map(i => ({ ...i, selected: i.optional ? ids.includes(i.id) : true })) })}/></div><TemplateRenderer q={q} template={template}/><ClientResponse key={JSON.stringify([q.commercial?.selectedPackage, q.items.filter(i => i.selected && i.optional).map(i => i.id)])} token={token} initialStatus={status as 'draft' | 'sent' | 'approved' | 'declined' | 'changes_requested'} initialNote={note} locked={locked} dueDateMode={q.commercial?.dueDateMode ?? 'fixed'} selection={{ selectedPackage: q.commercial?.selectedPackage ?? '', optionalIds: q.items.filter(i => i.optional && i.selected).map(i => i.id), revision }}/></>;
}
