...
const sleekInput = "mt-2 w-full bg-transparent border-0 border-b-2 border-white/10 px-0 py-2 text-xl font-light text-white placeholder:text-white/20 focus:border-[#FF6A00] focus:ring-0 transition-colors";
const sleekTextarea = "mt-2 w-full rounded-xl border border-white/10 bg-white/5 p-4 text-sm font-light text-white placeholder:text-white/20 focus:border-[#FF6A00] focus:ring-0 transition-colors backdrop-blur-sm";
const sleekSelect = "mt-2 w-full bg-transparent border-0 border-b-2 border-white/10 px-0 py-2 text-lg font-light text-white focus:border-[#FF6A00] focus:ring-0 transition-colors cursor-pointer appearance-none";
const labelStyle = "text-[9px] font-black uppercase tracking-[.25em] text-white/40";
...
export function CommercialEditor() {
  ...
  return (
    <motion.section 
      ...
      initial="hidden" 
      animate="show" 
      className="mb-24 space-y-12 text-white"
    >
      {/* HEADER */}
      <motion.div variants={fadeUp} className="border-b border-white/5 pb-8">
        ...
        <p className="mt-4 text-sm font-medium text-white/50 max-w-2xl leading-relaxed">
          A aparência é escolhida na próxima tela. Aqui você define o comportamento matemático da proposta: os escopos, os valores e a arquitetura de cobrança.
        </p>
      </motion.div>
        ...
        {COMMERCIAL_MODELS.map(m => {
          ...
          return (
            <button 
              ...
              className={`group relative overflow-hidden rounded-[2rem] border p-6 text-left transition-all duration-300 ${isSelected ? 'border-[#FF6A00]/50 bg-[#FF6A00]/10 shadow-[0_0_30px_rgba(255,106,0,0.15)] scale-[1.02]' : 'border-white/5 bg-white/[0.02] hover:bg-white/[0.04] hover:border-white/20 backdrop-blur-sm'}`}
            >
              {isSelected && (
                <div className="absolute -top-10 -right-10 h-32 w-32 rounded-full bg-[#FF6A00]/20 blur-[40px] pointer-events-none" />
              )}
              <strong className={`block font-display text-xl tracking-tight transition-colors ${isSelected ? 'text-white' : 'text-white/70 group-hover:text-white'}`}>
                {m.name}
              </strong>
              <span className="mt-3 block text-[11px] font-medium uppercase tracking-wider leading-relaxed text-white/40">
                {m.description}
              </span>
            </button>
          );
      ...
      <motion.div variants={fadeUp} className="relative overflow-hidden rounded-[2.5rem] border border-white/5 bg-black/40 p-8 shadow-2xl backdrop-blur-3xl sm:p-12">
        ...
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          <label className="flex flex-col">
            ...
            >
              <option value="fixed" className="bg-neutral-900 text-white">Data fixa</option>
              <option value="month_end" className="bg-neutral-900 text-white">Fim do mês</option>
              <option value="client" className="bg-neutral-900 text-white">Cliente escolhe no aceite</option>
            </select>
          </label>
          {(c.dueDateMode ?? 'fixed') === 'fixed' && (
            <label className="flex flex-col">
              <span className={labelStyle}>Primeiro Vencimento</span>
              <input type="date" className={sleekInput} value={c.firstDueDate} onChange={e => patch({ firstDueDate: e.target.value })} style={{ colorScheme: 'dark' }} />
            </label>
          )}
          <label className="flex flex-col">
        ...
        <div className="mt-6 flex items-center gap-3 rounded-xl border border-white/5 bg-white/5 px-5 py-3 text-xs font-medium text-white/50 w-fit">
          <Calendar size={14} className="text-[#FF6A00]" />
          O aceite gera as cobranças no portal. Parcelamento divide o valor único.
        </div>

        {c.model === 'packages' && (
          <div className="mt-10 grid gap-8 sm:grid-cols-3 border-t border-white/5 pt-10">
            {c.packages.map(p => (
        ...
        <div className="mt-12 grid gap-8 sm:grid-cols-2 border-t border-white/5 pt-10">
          <label className="flex flex-col">
      ...
      {draft.services.length > 0 && (
        <motion.div variants={fadeUp} className="space-y-6 pt-8">
          <div className="flex items-center gap-4 border-b border-white/5 pb-4">
            <h3 className="font-display text-3xl font-light text-white">Composição da Oferta</h3>
            ...
            {draft.services.map((s, idx) => {
              ...
              return (
                <motion.div 
                  ...
                  key={s.id} 
                  className="group relative rounded-[2rem] border border-white/5 bg-white/[0.02] p-8 backdrop-blur-xl transition-all hover:bg-white/[0.04] hover:border-white/10"
                >
                  <div className="flex items-start justify-between gap-4 border-b border-white/5 pb-6">
                    <div className="flex items-center gap-4">
                      <span className="flex h-10 w-10 items-center justify-center rounded-full bg-white/5 font-display text-lg font-light text-white/50">
                        {idx + 1}
                      </span>
                      <strong className="font-display text-2xl font-normal tracking-wide text-white">{s.name}</strong>
                    </div>
                    <div className="flex items-center gap-2 opacity-0 transition-opacity group-hover:opacity-100">
                      <button type="button" aria-label={`Duplicar ${s.name}`} onClick={() => update({ services: [...draft.services, { ...s, id: crypto.randomUUID(), name: s.name + ' - cópia' }] })} className="flex h-10 w-10 items-center justify-center rounded-full bg-white/5 text-white/50 hover:bg-white/10 hover:text-white transition-colors">
                        <Copy size={16} />
                      </button>
                    ...
                    {c.model === 'packages' && (
                      <label className="flex flex-col">
                        ...
                        <select className={sleekSelect} value={s.packageId ?? ''} onChange={e => edit({ packageId: e.target.value })}>
                          <option value="" className="bg-neutral-900 text-white">Todos</option>
                          {c.packages.map(p => <option key={p.id} value={p.id} className="bg-neutral-900 text-white">{p.name}</option>)}
                        </select>
                      </label>
                    )}
                  </div>
                  
                  <div className="mt-8 grid gap-8 sm:grid-cols-2 border-t border-white/5 pt-8">
                    <label className="flex flex-col">
                  ...
                  <label className="mt-8 flex w-fit cursor-pointer items-center gap-4 rounded-full border border-white/5 bg-white/5 px-6 py-3 transition-colors hover:bg-white/10">
                    <div className="relative flex h-5 w-5 items-center justify-center rounded-md border-2 border-white/20 bg-transparent transition-colors">
                    ...
                    <span className="text-xs font-bold uppercase tracking-widest text-white/60">Adicional Opcional</span>
                  </label>
                </motion.div>
              );
            })}
          ...
          <motion.div variants={fadeUp} className="relative overflow-hidden flex flex-wrap items-center justify-between gap-8 rounded-[2rem] bg-gradient-to-r from-[#FF6A00]/20 to-transparent border-l-4 border-[#FF6A00] p-8 shadow-2xl">
            ...
            <div className="relative z-10 flex gap-12">
              <div className="flex flex-col gap-1">
                ...
                <strong className="font-display text-3xl font-light text-white">{formatBRL(totals.once)}</strong>
              </div>
              <div className="flex flex-col gap-1 border-l border-white/10 pl-12">
                <span className={labelStyle}>Total Mensalidade</span>
                <strong className="font-display text-3xl font-light text-white">{formatBRL(totals.monthly)}</strong>
              </div>
            </div>
            <div className="relative z-10 flex flex-col items-end gap-1">
              <span className="text-[10px] font-black uppercase tracking-[.4em] text-[#FF6A00]">Impacto da Proposta</span>
              <strong className="font-display text-5xl font-normal tracking-tight text-white drop-shadow-[0_0_20px_rgba(255,106,0,0.4)]">
