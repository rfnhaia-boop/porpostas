          ...
          <div>
            <div className="flex items-center gap-4 mb-4">
              <div className="px-3 py-1 bg-brand-cyan/20 border border-brand-cyan/50 text-brand-cyan text-[10px] uppercase tracking-widest font-bold">
                PROPOSTA Nº {quoteDraft.proposalNumber}
              </div>
            ...
            <h1 className="text-6xl font-black tracking-tighter uppercase text-white mb-2">
              {companyInfo.name} <span className="text-brand-cyan text-shadow-neon">PROPOSTA</span>
            </h1>
            {companyInfo.cnpj && (
              <p className="text-brand-cyan/60 tracking-widest text-xs uppercase font-mono">CNPJ {companyInfo.cnpj}</p>
            )}
          </div>
        ...
        >
          ...
          <div className="flex justify-between items-start mb-12">
            <div className="max-w-xl">
              ...
              <p className="text-sm font-light text-white/60 leading-relaxed whitespace-pre-wrap">{quoteDraft.notes}</p>
            </div>
            <div className="text-right">
              ...
              <div className="flex flex-wrap gap-4 justify-end mt-4 text-[10px] font-mono uppercase text-brand-cyan/70">
                <p>VALIDADE: {quoteDraft.validityDays}</p>
                <p>PRAZO: {quoteDraft.timeline}</p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-8 grid grid-cols-3 gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 text-left">
            <div className="bg-[#080b0b] p-4"><p className="text-[9px] uppercase tracking-widest text-white/35">Validade</p><strong className="mt-1 block text-sm text-white">{quoteDraft.validityDays}</strong></div>
            <div className="bg-[#080b0b] p-4"><p className="text-[9px] uppercase tracking-widest text-white/35">Execução</p><strong className="mt-1 block text-sm text-white">{quoteDraft.timeline}</strong></div>
            <div className="bg-[#080b0b] p-4"><p className="text-[9px] uppercase tracking-widest text-white/35">Pagamento</p><strong className="mt-1 block text-sm text-white">{quoteDraft.paymentTerms || '50% na aprovação e 50% na entrega'}</strong></div>
          </div>
          <div className="border-t border-white/20 pt-8 flex justify-between items-end font-mono text-[10px] text-white/58 uppercase">
            <div className="flex flex-col gap-1">
              <p>EMITENTE: {companyInfo.name} [{companyInfo.cnpj}]</p>
              <p>CONTATO: {companyInfo.email} // {companyInfo.phone}</p>
            </div>
            <div className="text-right">
