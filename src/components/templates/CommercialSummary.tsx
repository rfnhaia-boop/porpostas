...
export function CommercialSummary({ q }: { q: QuoteView }) {
  ...
  try { if (c.dueDateMode !== 'client') schedule = commercialSchedule(q.items, c); } catch { /* Incomplete draft: the editor supplies validation. */ }
  return <section className="mx-auto w-full max-w-[21cm] bg-white px-6 py-10 text-[#17231f] sm:px-12 print:break-before-page print:px-0">
    ...
    {c.dueDateMode === 'client' && <p className="mt-4 text-sm">O calendário será confirmado com a data escolhida no aceite.</p>}
