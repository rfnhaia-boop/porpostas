...
export function GoogleButton({ label }: { label: string }) {
  ...
  const handleClick = async () => {
    ...
    if (error) {
      setError(error.message || 'Login com Google ainda não configurado.');
      setLoading(false);
    }
    // sucesso: o Better Auth redireciona pro Google — não precisa fazer nada aqui.
  };
  ...
  return (
    <div>
      <button
        ...
        onClick={handleClick}
        disabled={loading}
        className="w-full flex items-center justify-center gap-3 rounded-2xl border border-[var(--border-color)] bg-[var(--panel-bg)] p-4 text-sm font-bold text-[var(--foreground)] hover:border-[#FF6A00] transition-colors disabled:opacity-50"
      >
        <GoogleIcon />
